...
cross_val_evaluate(best_svr, X_train, y_train, "SVM")
...
def evaluate(y_true, y_pred, model_name, data_type):
    ...
    print(f"{model_name} {data_type}Train MSE: {mse:.4f}, MAE: {mae:.4f}, R²: {r2:.4f}")
...
y_train_pred = best_svr.predict(X_train)
y_test_pred = best_svr.predict(X_test)
plot_pred_vs_actual_full(y_train, y_train_pred, y_test, y_test_pred, "SVM")
...
plot_residuals(y_train, y_train_pred, "SVM on Train set")

# 测试集残差分析
print("\n-------------------Residuals analysis on test set--------------------------")
plot_residuals(y_test, y_test_pred, "SVM on Test set")
